...
export class MessageSummaryComponent implements OnInit {
  ...
  reqObj: any = {
    "fromDate": moment(this.filterDetailsObj.FromDate[0]).subtract(30, 'days').format("YYYY-MM-DD"),
    "pageIndex": 1,
    "noOfRecords":10,
...
page = 1;
  ...
  listCall(){
     //ListChannel
     this.apiService.post(this.constantsService.messageStatisticsSummary, this.reqObj).subscribe((succ: any) => {
  ...
  gotoMessageSatticsReport(one, two){
    console.log(two.SUCCESS, "SUCCESS")
    console.log(two.TOTAL, "TOTAL")
    ...
    this.router.navigateByUrl('/home/reports/app-message-statistics');
  ...
  downloadReport(format) {
    ...
    this.apiService.downloadFile(this.constantsService.messageStatisticsSummaryDownload, this.reqObj).subscribe((succ: any) => {
